drop function if exists analytics.one_players_ranking_enriched;

create function analytics.one_players_ranking_enriched(in_ranking character varying, in_comps character varying[], in_seasons character varying[], group_by_club boolean DEFAULT true, group_by_competition boolean DEFAULT true, group_by_season boolean DEFAULT true, first_week integer DEFAULT 0, last_week integer DEFAULT 100, first_date character varying DEFAULT '1970-01-01'::character varying, last_date character varying DEFAULT '2099-12-31'::character varying, day_slots character varying[] DEFAULT '{}'::character varying[], time_slots character varying[] DEFAULT '{}'::character varying[], side analytics.side DEFAULT 'both'::analytics.side, r integer DEFAULT 2)
    returns TABLE("Player" character varying, "Club" character varying[], "Competition" character varying[], "Season" character varying[], "Age" integer, "Height" integer, "Weight" integer, "Footed" character varying, "Nationalities" character varying[], "Matches" integer, "Involved Matches" integer, "Stat" numeric)
    language plpgsql
as
$$
DECLARE
...
-- Sinon : 'Competition'
	...
	with players_stats as (
		...
		group by
			(split_part(stats.id_player, '_', 1), stats.player, stats.nationalities),
